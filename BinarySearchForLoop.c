...
int binarySearchLoop(int n, int array[], int key) { //반복을 활용해 이진탐색하는 함수
	int mid;
	for (int i=0; i<=n;) { //i>n이 아닐 동안 반복
		mid = (i + n) / 2; //mid: 시작i와 끝n의 중간 지점 (반복하며 바뀐 i값 또는 n값에 따라 mid값 변경) 
		if (key==array[mid]) { //1) key와 mid인덱스의 array가 일치하는 경우, 리턴
			return mid;
		}
		else { //2) key와 mid인덱스의 array가 서로 다른 경우:
	...
	return -1; //3) key값을 찾지 못한 경우, -1 리턴 (실패)
}
...
int main(void) {
	...
	//2. 출력
	int result = binarySearchLoop(n-1, array, key) + 1; //배열인덱스를 고려해 탐색끝을 n-1로, result를 리턴값+1로 지정
